...
def Enviardef(conteudo):
	...
	packages = prepararParaEnvio(conteudo) #cria a lista final de pacotes a serem enviados
	windowRight = min(int(windowSize),qtdpacotes)#para não tentar enviar coisas que não existem na lista (segmentation fault)
	recebido = ""
	while True:
		if enviados == 0: # se for a primeira transmissão (poderia melhorar a lógica but deadline)
		...
		try:
			...
			if dados[2] == 1: #checka se é ACK
				...
				if Sn >= windowLeft and Sn < qtdpacotes: #numero de sequencia tem que estar entre o começo da janela e a qtd total de pacotes 
					...
					socketar(fin) # manda uma mensagem que indica pro server que as mensagens acabaram
...
def timedout(): 
	...
	global packages
	temp = windowLeft
	while temp <= windowRight and temp < qtdpacotes: #envia a janela toda dnv e volta pra esperar os acks :)
	...
	with open("sherlock.txt","rb") as arquivo :
		while True:
			...
			if fatia:
				...
				break
	log.write("Começando programa, Registrar LOG,data : " + strftime("%c ", gmtime()) + "\n")
	Enviardef(conteudo)

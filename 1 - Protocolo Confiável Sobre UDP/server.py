...
# Criar Socket para poupar Tempo (não ficar pegando o endereço toda vez
# que vai enviar algo)
    ...
    while True:
        ...
        if data == "QuitConnection":
            ...
            if ack == 1000:
                sleep(0.1)
            sendACK(ack)
            log.write("Recebido sequencia : " + str(sequenceNumber) +
                      "---- Ack enviado : " + str(ack) + "\n")
            with open("copia.txt", "ab") as output:
                output.write(data)
            expectedSequence += 1
        elif sequenceNumber != expectedSequence:
            log.write("Recebido numero de sequencia fora de ordem !! Sn = " +
                      str(sequenceNumber) + "\n")
